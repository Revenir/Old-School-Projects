...
# Program Name: birthdayalg.py
# Due Date: 9/16/2013
# Program description: Finds duplicate birthdays to prove the birthday paradox.
...
people = []                         # Make an empty list for the people
                                    # entering the room

for x in range(population):           
    month = random.randint(1, 12)   # A random month is chosen.
                                    # We need the if statements to make sure we have the
    ...
    if month == 1 or month == 3 or month == 5 or month == 7 or month == 8 or month == 10 or month == 12:
        ...
        day = random.randint(1, 28) # If we cared about leap years,
                                    # we could make this 29

    elif month == 4 or month == 6 or month == 9 or month == 11:
        day = random.randint(1, 30)

    person = month, day             # Creates a tuple, since birthdays are static
    people.append(person)           # Adds each person to the list of people

twins = set()                       # Since sets do not allow duplicates, this
                                    # prevents a birthday from showing
                                    # up more than once.
for birthdays in people:
    if people.count(birthdays) > 1: # Counts the number of instances an element appears
        twins.add(birthdays)        # Adds duplicate birthdays to a list.
        
twins = list(twins)                 # Sets don't support indexing, which we need.
for x in twins:
    if x[0] == 1:                   # Prints out the actual name of the month
...
if not twins:                       # If the list is empty, prints out a different message.
